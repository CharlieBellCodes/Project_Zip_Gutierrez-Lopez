...
There were missing/NA variables due to incomplete or cancelled Uber rides, so these were dropped. The dimension of the data is 150000 Rows by 21 Columns. Additionally, all variables were under the character type and some have to be converted to their correct types (numerical or datetime).
...
```
In this data set, date and time need to be converted into datetime variables. Additionally, average vehicle turn around time, booking value, ride distance, and customer rating need to be converted into numerical variables to make it easier to plot.
...
The rows that have no customer ratings need to be dropped, as the main goal of this project is to determine what factors positively or negatively affect customer ratings.
...
rounded_data <- final_data %>%
  mutate(rounded_date = round_date(Date,
             unit = "month"))
              #create new column round the dates to the end of their week
```
...
```
This bar chart shows the frequency of booking dates based on months and years. The data seems to be pretty uniform with the exception of the first and last bars, Jan 2024 and Jan 2025. It is odd that there are more bookings for January 2024 as opposed to January 2025, but the difference is very slight.
...
```
This boxplot shows the vehicle turn around times for the dataset. The minimum is 2 minutes and the maximum is around 15 minutes. The mean is around 8.5 minutes, the first quartile is at 5.5 minutes and the third quartile is at approximately 12 minutes. It seems like there is not that long of a waiting time for a vehicle, and there does not seem to be any outliers in this graph.
...
```
This scatter plot shows the most popular booking times and their respective booking value. After around 9 am, there seems to be an increase in average booking value, from around 1000 to around 1500. However, all times seem to have at least one high booking value, this may be because of the type of vehicle or destination as well. The more frequent booking values tend to be at or under 1000. As the booking values increase, their frequency decrease.
...
```
This is a line graph of the frequency of ride distance for each Uber ride. It seems like it is consistent in that distances from 5-45 miles are all around the same frequency. It seems like consumers take Uber rides to a variety of different locations.
...
This boxplot shows average customer ratings for the data set. It seems like the overall rating for these rides is 4.5, which is a high average. There are a few lower ratings as outliers, however, those seem to not be the standard. It is usually around a 4-4.7 rating.
...
```
This scatter plot graph compares how long an Uber ride was and what the customer's rating was. There does not seem to be a correlation here. All different ride distances seem to have had every customer rating (or most of them), so ride distance does not seem to influence how a customer rates their ride.
...
final_data %>%
  ...
  theme (axis.title.x = element_text(color = "white"),axis.title.y = element_text(color = "white"),axis.text.x = element_text(color = "white"),axis.text.y = element_text(color = "white"),panel.background = element_rect(fill = "#ffddfc"), plot.background = element_rect(fill = "#e88bff", color ="#f7a0d6"),title = element_text(color = "white"))
```
This boxplot graph shows the average customer ratings per vehicle type booked. It does not appear that vehicle type has any affect on the customer rating given to each ride. All types of vehicles seem to have an average of a rating of around 4.5, with Auto, Bike, Premier Sedan, and Uber XL only barely not meeting the 4.5 average. For all types of vehicles the average is high and the outliers are sparing, with the most being around 3.2. There does not appear to be any correlation between the two as all the box plots appear similar, so vehicle types do not seem to influence how a customer rates their ride.
...
This scatter plot graph compares the average turn around time for an Uber ride and what the customer's rating was. There seems to be no correlation here. All different turn around times seem to have had every customer rating (or most of them), so average vehicle turnaround time does not seem to influence how a customer rates their ride.
...
final_data %>%
  ...
  theme (axis.title.x = element_text(color = "white"),axis.title.y = element_text(color = "white"),axis.text.x = element_text(color = "white"),axis.text.y = element_text(color = "white"),panel.background = element_rect(fill = "#ffddfc"), plot.background = element_rect(fill = "#e88bff", color ="#f7a0d6"),title = element_text(color = "white"))
```
This scatter plot graph compares what month and year an Uber ride was booked and what the customer's rating was. There seems to be no correlation here. All different months and years seem to have had every customer rating (or most of them), so booking date does not seem to influence how a customer rates their ride.
...
```
This scatter plot graph compares at what time an Uber ride was booked and what the customer's rating was. There seems to be no correlation here. All the different hours seem to have had every customer rating (or most of them), so the time of booking does not seem to influence how a customer rates their ride.
...
```
This correlation matrix identifies if there are any relationships between ride distance, average vehicle turn around time, booking value, and customer rating. This matrix shows that there are no significant relationships between the variables, as all variables have less than .01 correlation with each other (with the exception of the variables compared to themselves).


# Results

This  analysis focused on determining if there was any significant linear correlation between several numerical variables believed to be important to overall customer ratings The method involved analyzing certain variables by themselves and then again against consumer ratings. To compare these variables against consumer ratings, scatter plots were used to  inspect the data for any trends or patterns. Then the correlation coefficient was calculated for each variable with the consumer rating.

Scatter plots serve as a great visual aid that can demonstrate a trend. The calculation of the correlation coefficient shows a numerical value that can help determine if there were any actual trends. A coefficient near 1 indicates a strong relationship, while a value close to 0 confirms their is none. 

The initial assumption that these selected variables would show a strong connection to customer ratings was wrong. 
